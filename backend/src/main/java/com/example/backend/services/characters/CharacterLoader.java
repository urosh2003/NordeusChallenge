package com.example.backend.services.characters;
...
public class CharacterLoader {
    ...
    public List<CharacterDefinition> getEnemies() {
        return definitions.values().stream()
                .filter(def -> !def.isStartingClass() && !def.isBoss())
                .collect(Collectors.toList());
    }
    public List<CharacterDefinition> getBosses() {
        return definitions.values().stream()
                .filter(CharacterDefinition::isBoss)
                .collect(Collectors.toList());
    }
