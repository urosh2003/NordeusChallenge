...
#### `backward-vulnerability.drl` — Backward chaining upiti

| Upit | Parametri | Dokazuje |
|------|-----------|----------|
| `canKillPlayer` | playerHp | Postoji `MoveOption` čiji `projectedValue >= playerHp` |
| `canKillPlayerInNTurns` | playerHp, turnsLeft | Rekurzivno: neprijatelj može ubiti igrača za N poteza koristeći maksimalnu štetu po potezu |
| `isVulnerableTo` | entityId, damageType | Entitet ima aktivan debuff na odbrani ILI ima urođeno nisku odbranu (≤ 4) |
...
| `ChooseFinishSoonTactic` | 60 | `PerceivedThreat(LOW)` + `?canKillPlayerInNTurns(playerHp, 2)` uspešan | INSERT `Tactic(MAXIMIZE_DAMAGE)` |
...
| `ChooseConservativeTactic` | 45 | `ResourceStatus(STARVED)` | INSERT `Tactic(CONSERVATIVE)` |
